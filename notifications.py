...
# In notifications.py, replace the existing NotificationRow class with this one.
...
class NotificationRow(Gtk.ListBoxRow):
    ...
    def create_ui(self):
        # The main, always-visible header part of the notification.
        ...
        # This box will now only contain the app name and summary.
        # It will expand to push the time and arrow to the right.
        content_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4, 
                              hexpand=True, # This is crucial for alignment
        ...
        # === ALIGNMENT FIX: The time label is now a direct child of the main header_box ===
        # This allows it to be vertically centered relative to the whole row.
        time_label = Gtk.Label(label=self.format_timestamp(),
                               valign=Gtk.Align.CENTER, # Vertically center the text
                               margin_end=6,           # Add a small space before the arrow
        ...
        # Build the header box in the correct order
        header_box.append(self.avatar)
        header_box.append(content_box) # The expanding content
        header_box.append(time_label)  # The correctly aligned time
    ...
    def format_timestamp(self):
        """
        Formats the notification timestamp into a human-readable relative time,
        with specific formatting for notifications older than 1 hour.
        """
        ...
        try:
            ...
            # === FORMATTING FIX: New logic as per your request ===
            if seconds < 60:
                return "now"
            elif seconds < 3600:
                return f"{int(seconds // 60)}m ago"
            elif diff.days == 0:
                # Older than an hour, but still today: show HH:MM
                return ts.strftime('%H:%M')
            elif diff.days == 1:
                return "yesterday"
            else:
                # Older than yesterday: show Month Day
...
class NotificationsWidget(Gtk.Box):
    ...
    # ... (rest of NotificationsWidget class is unchanged and correct) ...
